...
按 `'` 打开默认居中的自绘语音工作台，拖动标题栏可移动窗口。窗口不使用游戏默认控件样式，命令图标来自随包分发的 Lucide SVG，并按“音频 / 频道 / 服务器管理 / 状态”分区；服务器管理页只对有权限的玩家显示。控件停留 300ms 后在页脚显示一行短说明，不绘制跟随鼠标的提示浮层；长设备、频道和百人玩家列表使用分页选择浮层。完整设计约束见 `docs/UI-DESIGN.md`。
...
- 频道所有者或管理员可在自绘输入框中修改持久频道名称；外部文明提供器管理的频道保持只读。
- 具备服务器管理权限时，可先输入名称，再从独立管理页创建文明/指挥/外交/工作人员/广播/无线电频道，并执行临时或持久语音管制。
- 接受/拒绝小队邀请，离开或解散频道。
