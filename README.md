...
- 调试录音：录制 3 秒本机麦克风，并播放经过发送前处理、ADPCM 编码/解码和当前游戏环境音效后的结果。

切换输入设备后会自动重新打开麦克风采集；如果设备不可用，会在聊天栏提示，仍可听到其他玩家语音。

调试录音只在本地使用，不会发送给服务器或其他玩家。它适合测试真实游戏里的麦克风链路、噪声门、增益，以及水下、遮挡、洞穴、风雨、时间稳定性等接收端效果是否明显。
...
- 距离越远，声音会逐渐更闷，不只是变小。
- 隔墙/地形遮挡会降低音量并增加更明显的低通闷声。
- 水下会增加明显闷声、轻微失真和音高变化。
- 室内/洞穴会根据周围方块包围度和天空光估算可感知的短回音。
- 暴风雨和大风会在露天时降低传播清晰度，增加闷声、失真和抖动。
- 时间稳定性较低、中毒状态能被属性识别时，会增加 pitch、闷声、抖动或失真效果。
- `/svc status` 和状态窗口会显示本地环境诊断，例如水下、室内/洞穴、风雨、稳定度、中毒、低通和回音数值，方便确认效果是否触发。
...
- 播放端使用低延迟模式：20ms 更新、较小 jitter buffer，并会丢弃麦克风积压旧采样以避免局域网测试出现秒级延迟。
